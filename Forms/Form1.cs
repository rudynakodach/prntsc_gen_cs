...
namespace prntsc_gen
{
	public partial class Form1 : Form
	{
		...
		public const string imageSaveDirectory = "/images/";
		...
		private void ButtonGenerate_Click(object sender, EventArgs e)
		{
			...
			if (WebBrowser.GetDirectImageLink(new Uri(currentLink)) != null)
			{
				if (!WebBrowser.Get(WebBrowser.GetDirectImageLink(new Uri(currentLink)), AppStatusLabel))
				{
			...
			if (AutoPreviewCheckbox.Checked)
			{
				...
				string directImageLink = WebBrowser.GetDirectImageLink(currentLinkUri);
				...
				{
					webBrowser1.Url = new Uri(WebBrowser.GetDirectImageLink(currentLinkUri));
				}
			}
		}
		...
		public static class WebBrowser
		{
			...
			public static string GetDirectImageLink(Uri url)
			{
		...
		private void PreviewLinkButton_Click(object sender, EventArgs e)
		{
			try
			{
				webBrowser1.Url = new Uri(WebBrowser.GetDirectImageLink(new Uri(currentLink)));
			}
		...
		private void testbutton_Click(object sender, EventArgs e)
		{
			try
			{
				WebBrowser.GetDirectImageLink(new Uri(currentLink));
			}
		...
		{
			Clipboard.SetText(WebBrowser.GetDirectImageLink(new Uri(currentLink)));
		}
		...
		internal async Task FormFadeIn()
		{
			try
			{
				if (ActiveForm != null)
				{
					while (ActiveForm.Opacity < 1)
					{
						ActiveForm.Opacity += 0.01d;
						await Task.Delay(1);
			...
			{
				AppStatusLabel.Text = ex.Message;
			}
		}
		private void DownloadImageButton_Click(object sender, EventArgs e)
		{
			if(currentLink != null)
			{
				...
				string CurrentDirectImageFilename = WebBrowser.GetDirectImageLink(new Uri(currentLink));
				if(File.Exists(CurrentDirectImageFilename))
				{
					return; //nie pobieraj tego samego
				}
				try
				{
					wc.DownloadFile(CurrentDirectImageFilename, GetURLFilename(currentLink));
				}
				catch ( Exception ex)
				{
					AppStatusLabel.Text = ex.Message;
